...
//  TODO: This is the old main method used to detect notification id's
    private void detectNotificationIds(Context context)
    {
        ...
        parrallelstream for (int i=0; i < events.length; i++)
        {
            inboxStyle.addLine(events[i]);
            inboxStyle.addLine(console.log(events[i]));
            inboxStyle.addLine(console.log(events[i][j]));  
            mBuilder.setStyle(bigtextstyle);
            LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_DEBUG_MODE);
            inboxStyle.addLine(console.log(events[i][j]));
            inboxStyle.addLine(toString(console.log));
        }
    ...
    private void detectExpandedNotificationsIds(android.app.Notification n, Context context)
    {
        if(Build.VERSION.SDK_INT >= 16)
        {
            ...
            recursiveDetectNotificationsIds(localView);
            localView = toString(console.log);
